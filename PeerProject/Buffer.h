...
public:
    inline DWORD GetBufferSize() const {
        return m_nBuffer;    // Return the total size of the buffer
    }
    inline BYTE *GetData() const {
        return m_pBuffer;    // Return a pointer to the start of the data in the buffer
    }
    inline DWORD GetCount() const {
        return m_nLength;    // Return the filled size of the buffer
    }
    inline BYTE *GetDataEnd() const {
        return m_pBuffer + m_nLength;    // Return a pointer to the end of the data in the buffer
    }
    inline DWORD GetBufferFree() const {
        return m_nBuffer - m_nLength;    // Return the unused #bytes in the buffer
    }
...
public:
    ...
    DWORD	AddBuffer(CBuffer *pBuffer) {
        return AddBuffer( pBuffer, pBuffer->m_nLength );
    }

    // Add ASCII text to the buffer
    void	Print(const LPCSTR pszText, const size_t nLength) {
        Add( (void *)pszText, nLength );
    }
    void	Print(const CStringA &strText) {
        Print( (LPCSTR)strText, strText.GetLength() );
    }

    // Convert Unicode text to ASCII and add it to the buffer
    void	Print(const CStringW &strText, const UINT nCodePage = CP_ACP) {
    ...
    void	Prefix(LPCSTR pszText, const size_t nLength) {
