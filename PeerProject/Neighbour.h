...
public:
    ...
    virtual DWORD GetUserCount() const {
        return 0;    // Returns hub/server leaf/user count variable
    }
    virtual DWORD GetUserLimit() const {
        return 0;    // Returns hub/server leaf/user limit variable
    }
    ...
    virtual BOOL ProcessPackets(CBuffer * /*pInput*/) {
        return TRUE;    // Process packets from input buffer
    }

protected:
    virtual BOOL ProcessPackets() {
        return TRUE;    // Process packets from internal input buffer
    }
