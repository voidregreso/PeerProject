...
void CQueryHashTable::MakeKeywords(const CString &strPhrase, CStringList &oKeywords) {
    ...
    for ( ; nPos < strPhrase.GetLength() ; ++nPos ) {
        ...
        // Work-around for WinXP: Katakana/Hiragana diacritic characters in the
        // NLS table aren't marked as being alpha (Win2000 & Vista+ are OK)
...
void CQueryHashTable::MakeKeywords(const CString &strWord, WORD nWordType, CStringList &oKeywords) {
    ...
    if ( nWordType & C3_HIRAGANA ) {
        // Continuous Hiragana string can be structured with a few prefix or postfix
        // or both. Assume Prefix/Postfix length is MAX 2 chars.
    ...
    if ( nWordType & C3_KATAKANA ) {
        ...
        // Assume MAX number of Words contained in one continuous string as Two words.
...
void CQueryHashTable::AddFile(const CPeerProjectFile &oFile) {
    if ( ! m_pHash )
        return;
    AddHashes( oFile );
...
void CQueryHashTable::AddHashes(const CPeerProjectFile &oFile) {
    if ( ! m_pHash )
        return;
    if ( oFile.m_oSHA1 )
