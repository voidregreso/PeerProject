...
BOOL CG2Packet::OnCrawlRequest(const SOCKADDR_IN *pHost) {
    ...
    pPacket->WritePacket(
        G2_PACKET_SELF,
        16 + ( ! strNick.IsEmpty() ? pPacket->GetStringLen( strNick ) + 6 : 0 ) +
        ( nGPS ? 5 + 4 : 0 ) + ( ! vendorCode.IsEmpty() ? pPacket->GetStringLen( vendorCode ) + 3 : 0 ) +
        ( ! currentVersion.IsEmpty() ? pPacket->GetStringLen( currentVersion ) + 4 : 0 ) +
        ( bIsHub ? 5 : 6 ),
        TRUE );
...
BOOL CG2Packet::OnKHLA(const SOCKADDR_IN *pHost) {
    if ( ! m_bCompound ) {
        Statistics.Current.Gnutella2.Dropped++;
        return FALSE;	// If not Compound packet, it is basically malformed packet
    }
