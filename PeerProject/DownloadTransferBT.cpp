...
CString CDownloadTransferBT::GetStateText(BOOL bLong) {
    if ( m_nState == dtsTorrent ) {
        if ( ! m_bInterested )
            return LoadString( IDS_STATUS_UNINTERESTED );
        else if ( m_bChoked )
            return LoadString( IDS_STATUS_CHOKED );
        else
            return LoadString( IDS_STATUS_REQUESTING );
    }
...
BOOL CDownloadTransferBT::OnRun() {
    ...
    if ( ( tNow >= m_tSourceRequest + Settings.BitTorrent.SourceExchangePeriod * 60 * 1000 ) &&
            m_pDownload->GetSourceCount() < Settings.Downloads.SourcesWanted ) {
...
BOOL CDownloadTransferBT::OnHave(CBTPacket *pPacket) {
    ASSUME_LOCK( Transfers.m_pSection );
    if ( pPacket->GetRemaining() != 4 )	// sizeof( int )
        return TRUE;
    QWORD nBlockSize	= m_pDownload->m_pTorrent.m_nBlockSize;
    DWORD nBlockCount	= m_pDownload->m_pTorrent.m_nBlockCount;
