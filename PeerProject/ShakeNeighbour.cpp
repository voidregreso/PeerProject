...
// Tell the remote computer we are PeerProject, and setup Gnutella2 or just Gnutella communications
void CShakeNeighbour::SendMinimalHeaders() {
    // Tell the remote we are running PeerProject
    Write( _P("User-Agent: ") );
...
void CShakeNeighbour::SendPublicHeaders() {
    ...
    Write( _P("\r\n") );
    // PeerProject Settings allow us to exchange compressed data with this computer
    if ( m_bCanDeflate ) {
...
void CShakeNeighbour::SendHostHeaders(LPCSTR pszMessage, size_t nLength) {
    ...
    if ( pszMessage ) {
        ...
        Write( _P("\r\n") );
        SendMinimalHeaders();	// Say we are PeerProject, and understand Gnutella2 packets
    }
...
BOOL CShakeNeighbour::OnHeaderLine(CString &strHeader, CString &strValue) {
    ...
    case 'u':		// "User-Agent"
        ...
        // Record if the remote computer is running PeerProject/Shareaza
...
BOOL CShakeNeighbour::OnHeadersCompleteG1() {
    ...
    if ( ! Settings.Gnutella1.Enabled && m_nState < nrsRejected ) {	// And make sure the state is before getting rejected
        ...
        SendMinimalHeaders();		// Tell the remote computer we're PeerProject and we can exchange Gnutella2 packets
...
void CShakeNeighbour::OnHandshakeComplete() {
    ...
    // Point the bandwidth meter limits at the numbers from PeerProject Settings
    switch ( m_nNodeType ) {
    case ntHub:
