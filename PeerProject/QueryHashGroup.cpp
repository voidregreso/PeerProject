...
void CQueryHashGroup::Operate(CQueryHashTable *pTable, BOOL bAdd) {
    ...
    if ( bAdd ) {
        for ( DWORD nHash = m_nHash >> 3 ; nHash ; nHash-- ) {
            ...
            if ( ( nSource & 0x01 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x02 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x04 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x08 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x10 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x20 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x40 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
            if ( ( nSource & 0x80 ) == 0 ) (*pTarget++) ++;
            else pTarget++;
        }
    } else {
        for ( DWORD nHash = m_nHash >> 3 ; nHash ; nHash-- ) {
