...
BOOL CG1Neighbour::OnPacket(CG1Packet *pPacket) {
    ...
    case G1_PACKET_PING:
        return OnPing( pPacket );			// Ping
    case G1_PACKET_PONG:
        return OnPong( pPacket );			// Pong, response to a ping
    case G1_PACKET_BYE:
        return OnBye( pPacket );			// Bye message
    case G1_PACKET_QUERY_ROUTE:
        return OnCommonQueryHash( pPacket ); // Common query hash
    case G1_PACKET_VENDOR:
    case G1_PACKET_VENDOR_APP:
        return OnVendor( pPacket );			// Vendor-specific message
    case G1_PACKET_PUSH:
        return OnPush( pPacket );			// Push open a connection
    case G1_PACKET_QUERY:
        return OnQuery( pPacket );			// Search query
    case G1_PACKET_HIT:
        return OnHit( pPacket );			// Hit, a search result
    }
