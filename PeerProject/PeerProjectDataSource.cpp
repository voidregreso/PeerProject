...
void CPeerProjectDataSource::Clean() {
...
template < typename T >
UINT CPeerProjectDataSource::DragDropThread(LPVOID param) {
...
HRESULT CPeerProjectDataSource::ObjectToURL(IDataObject *pIDataObject, CString &str) {
...
HRESULT CPeerProjectDataSource::ObjectToFiles(IDataObject *pIDataObject, CList < CString > &oFiles) {
    ASSERT( pIDataObject != NULL );
...
HRESULT CPeerProjectDataSource::SetDropEffect(IDataObject *pIDataObject, DWORD dwEffect) {
    ASSERT( pIDataObject != NULL );
...
BOOL CPeerProjectDataSource::IsPeerProjectObject(IDataObject *pIDataObject) {
    ASSERT( pIDataObject != NULL );
...
HRESULT CPeerProjectDataSource::Add(IDataObject *pIDataObject) {
    FORMATETC formatetc = { (CLIPFORMAT) RegisterClipboardFormat( CF_PEERPROJECT ), NULL, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
    STGMEDIUM medium = { TYMED_HGLOBAL, NULL, NULL };
...
HRESULT CPeerProjectDataSource::Add(IDataObject *pIDataObject, HBITMAP pImage, const CPoint &ptOffset) {
...
template < typename T >
HRESULT CPeerProjectDataSource::AddFiles(IDataObject *pIDataObject, const T *pSelFirst, const Hashes::Guid &oGUID ) {
    // Precalculate sizes
    size_t size_HDROP = 0;
    ...
    // Finalize CF_PEERPROJECT_ALBUMS
    if ( size_Archive ) {
        STGMEDIUM medium_Archive = { TYMED_ISTREAM, NULL, NULL };
        FORMATETC formatetc_Archive = { (CLIPFORMAT) RegisterClipboardFormat( CF_PEERPROJECT_ALBUMS ), NULL, DVASPECT_CONTENT, -1, TYMED_ISTREAM };
        buf_Archive.Close();
...
HRESULT CPeerProjectDataSource::FindFORMATETC(FORMATETC *pfe, LPDATAENTRY *ppde, BOOL fAdd) {
    ASSERT( pfe != NULL );
    ASSERT( ppde != NULL );
    *ppde = NULL;
...
HRESULT CPeerProjectDataSource::AddRefStgMedium(STGMEDIUM *pstgmIn, STGMEDIUM *pstgmOut, BOOL fCopyIn) {
    ASSERT( pstgmIn != NULL );
...
INTERFACE_PART(CPeerProjectDataSource, IID_IDragSourceHelper, DragSourceHelper)
END_INTERFACE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CPeerProjectDataSource data source interface

IMPLEMENT_UNKNOWN(CPeerProjectDataSource, DataObject)

STDMETHODIMP CPeerProjectDataSource::XDataObject::GetData(FORMATETC *pformatetc, STGMEDIUM *pmedium) {
...
STDMETHODIMP CPeerProjectDataSource::XDataObject::QueryGetData (FORMATETC *pformatetc) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DataObject )
    if ( pformatetc == NULL )
        return E_POINTER;
    LPDATAENTRY pde = NULL;
...
STDMETHODIMP CPeerProjectDataSource::XDataObject::SetData(FORMATETC *pformatetc, STGMEDIUM *pmedium, BOOL fRelease) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DataObject )
    if ( pformatetc == NULL )
        return E_POINTER;
    if ( pmedium == NULL )
...
STDMETHODIMP CPeerProjectDataSource::XDataObject::EnumFormatEtc(DWORD /*dwDirection*/, IEnumFORMATETC **ppenumFormatEtc) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DataObject )
...
STDMETHODIMP CPeerProjectDataSource::XDataObject::DAdvise(FORMATETC *pformatetc, DWORD advf, IAdviseSink *pAdvSink, DWORD *pdwConnection) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DataObject )
    HRESULT hr = S_OK;
...
STDMETHODIMP CPeerProjectDataSource::XDataObject::DUnadvise(DWORD dwConnection) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DataObject )
    HRESULT hr = S_OK;
    if ( m_spDataAdviseHolder == NULL )
...
STDMETHODIMP CPeerProjectDataSource::XDataObject::EnumDAdvise(IEnumSTATDATA **ppenumAdvise) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DataObject )
...
/////////////////////////////////////////////////////////////////////////////
// CPeerProjectDataSource drop source interface

IMPLEMENT_UNKNOWN(CPeerProjectDataSource, DropSource)

STDMETHODIMP CPeerProjectDataSource::XDropSource::QueryContinueDrag(BOOL fEscapePressed, DWORD grfKeyState) {
...
IMPLEMENT_UNKNOWN(CPeerProjectDataSource, DragSourceHelper)

STDMETHODIMP CPeerProjectDataSource::XDragSourceHelper::InitializeFromBitmap(LPSHDRAGIMAGE pshdi, IDataObject *pDataObject) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DragSourceHelper )
    return ( pThis->m_pdsh.p ) ? pThis->m_pdsh->InitializeFromBitmap( pshdi, pDataObject ) : E_NOTIMPL;
}

STDMETHODIMP CPeerProjectDataSource::XDragSourceHelper::InitializeFromWindow(HWND hwnd, POINT *ppt, IDataObject *pDataObject) {
    METHOD_PROLOGUE( CPeerProjectDataSource, DragSourceHelper )
...
void CPeerProjectDataSource::GetTotalLength(const CLibraryList *pList, size_t &size_HDROP, size_t &size_Archive, size_t &size_Files, BOOL bRoot) {
    ASSERT_VALID( pList );
...
void CPeerProjectDataSource::GetTotalLength(const CLibraryTreeItem *pSelFirst, size_t &size_HDROP, size_t &size_Archive, size_t &size_Files, BOOL bRoot) {
...
void CPeerProjectDataSource::FillBuffer(const CLibraryList *pList, LPTSTR &buf_HDROP, CArchive &buf_Archive, LPBYTE &buf_Files, CString &buf_Text, BOOL bRoot, const Hashes::Guid &oGUID) {
    ASSERT_VALID( pList );
    POSITION pos = pList->GetHeadPosition();
    while ( pos ) {
        CLibraryListItem Item = pList->GetNext( pos );
...
void CPeerProjectDataSource::FillBuffer(const CLibraryTreeItem *pSelFirst, LPTSTR &buf_HDROP, CArchive &buf_Archive, LPBYTE &buf_Files, CString &buf_Text, BOOL bRoot, const Hashes::Guid & /*oGUID*/) {
    ASSERT_VALID( pSelFirst );
    for ( const CLibraryTreeItem *pItem = pSelFirst ; pItem ; pItem = pItem->m_pSelNext ) {
        if ( pItem->m_pVirtual && bRoot &&
                ! CheckURI( pItem->m_pVirtual->m_sSchemaURI, CSchema::uriGhostFolder ) ) {
