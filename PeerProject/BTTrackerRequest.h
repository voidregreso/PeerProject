...
public:
    ...
    inline BTTrackerEvent GetEvent() const {
        return m_nEvent;    // Tracker event (update, announce, etc.)
    }
    inline DWORD	GetComplete() const {
        return m_nComplete;    // Seeders
    }
    inline DWORD	GetIncomplete() const {
        return m_nIncomplete;    // Leeches
    }
    inline DWORD	GetDownloaded() const {
        return m_nDownloaded;    // Download Count
    }

    // Tracker request interval (in seconds)
    inline DWORD	GetInterval() const {
        return min( max( m_nInterval, 60ul * 2ul ), 60ul * 60ul );
    }

    // Retrieve peers
    inline POSITION	GetSources() const {
        return m_pSources.GetHeadPosition();
    }
    inline const CBTTrackerSource &GetNextSource(POSITION &rPosition) const {
