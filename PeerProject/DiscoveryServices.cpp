    ...
    if ( pFile.Open( strFile, CFile::modeRead ) ) {		// Load default list from file if possible
        ...
        try {
            ...
            while ( pBuffer.ReadLine( strLine ) ) {
                ...
                case '1':
                    Add( strService, CDiscoveryService::dsWebCache, PROTOCOL_G1 );	// G1 service
                    break;
                case '2':
                    Add( strService, CDiscoveryService::dsWebCache, PROTOCOL_G2 );	// G2 service
                    break;
                case 'M':
                    Add( strService, CDiscoveryService::dsWebCache );					// Multinetwork service
                    break;
                case 'D':
                    Add( strService, CDiscoveryService::dsServerList, PROTOCOL_ED2K ); // eDonkey service
                    break;
                case 'H':
                    Add( strService, CDiscoveryService::dsServerList, PROTOCOL_DC );	// DC++ Hublist
                    break;
                case 'U':
                    Add( strService, CDiscoveryService::dsGnutella );					// Bootstrap and UDP Discovery Service
                    break;
                case 'X':
                    Add( strService, CDiscoveryService::dsBlocked );					// Blocked service
                    break;
                case '#':																	// Comment line
                    break;
                }
