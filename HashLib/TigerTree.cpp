...
#else // No HASHLIB_USE_ASM (x64)

namespace {
    const uint64 CTigerTree_m_pTable[4][256] = {
        {
...
void CTigerTree::SetupAndAllocate(uint32 nHeight, uint64 nLength) {
...
void CTigerTree::BeginFile(uint32 nHeight, uint64 nLength) {
    CSectionLock oLock( &m_pSection );
...
void CTigerTree::AddToFile(LPCVOID pInput, uint32 nLength) {
    CSectionLock oLock( &m_pSection );
    LPBYTE pBlock = (LPBYTE)pInput;
    while ( nLength > 0 ) {
        uint32 nBlock = min( nLength, BLOCK_SIZE );
        Tiger( pBlock, (uint64)nBlock, m_pStackTop->value );
        m_pStackTop ++;
        uint32 nCollapse = ++m_nBlockPos;
        while ( ! ( nCollapse & 1 ) ) {
            Collapse();
            nCollapse >>= 1;
        }
...
void CTigerTree::AddToTest(LPCVOID pInput, uint32 nLength) {
    CSectionLock oLock( &m_pSection );
    LPBYTE pBlock = (LPBYTE)pInput;
    while ( nLength > 0 ) {
        uint32 nBlock = min( nLength, BLOCK_SIZE );
...
BOOL CTigerTree::FinishBlockTest(uint32 nBlock) {
    CSectionLock oLock( &m_pSection );
...
BOOL CTigerTree::FromBytes(const uint8 *pInput, uint32 nInput, uint32 nHeight, uint64 nLength) {
    CSectionLock oLock( &m_pSection );
    SetupAndAllocate( nHeight, nLength );
    CTigerNode *pBase = m_pNode + m_nNodeCount - m_nNodeBase;
