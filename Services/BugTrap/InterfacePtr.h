...
public:
    ...
    CInterfacePtr(int iNull) {
        _ASSERTE(iNull == 0);
        iNull;
        m_ptr = NULL;
    }
    CInterfacePtr(INTERFACE *ptr) {
        Assign(ptr);
    }
    CInterfacePtr(const CInterfacePtr &iptr) {
        Assign(iptr.m_ptr);
    }
    ~CInterfacePtr(void) {
        Free();
    }
    const CInterfacePtr &operator=(INTERFACE *ptr) {
        Free();
        Assign(ptr);
        return *this;
    }
    const CInterfacePtr &operator=(const CInterfacePtr &iptr) {
        Free();
        Assign(iptr.m_ptr);
        return *this;
    }
    ...
    operator INTERFACE *(void) const {
        return m_ptr;
    }

    friend bool operator==(const CInterfacePtr &iptr1, const CInterfacePtr &iptr2) {
        return iptr1.m_ptr == iptr2.m_ptr;
    }
    friend bool operator!=(const CInterfacePtr &iptr1, const CInterfacePtr &iptr2) {
        return iptr1.m_ptr != iptr2.m_ptr;
    }
    ...
    HRESULT QueryInterface(INTERFACE2 *&ptr) const {
        _ASSERTE(m_ptr != NULL);
        m_ptr->QueryInterface(__uuidof(INTERFACE2), (void **)&ptr);
    }
    template <class INTERFACE2>
    HRESULT QueryInterface(CInterfacePtr<INTERFACE2> &iptr2) const {
        _ASSERTE(m_ptr != NULL);
        _ASSERTE(iptr2.m_ptr == NULL);
        m_ptr->QueryInterface(__uuidof(INTERFACE2), (void **)&iptr2.m_ptr);
    }
    ...
    HRESULT CreateInstance(REFCLSID rclsid, LPUNKNOWN pUnkOuter = NULL, DWORD dwClsContext = CLSCTX_ALL) {
        _ASSERTE(m_ptr == NULL);
        return CoCreateInstance(rclsid, pUnkOuter, dwClsContext, __uuidof(INTERFACE), (void **)&m_ptr);
    }
    ...
    template <class INTERFACE2>
    bool IsEqualObject(const CInterfacePtr<INTERFACE2> &iptr2) const {
        return IsEqualObject(iptr2.m_ptr);
    }

private:
    void Assign(INTERFACE *ptr) {
        if (ptr) ptr->AddRef();
        m_ptr = ptr;
    }
