...
void CSymEngine::GetOsInfo(COsInfo &rOsInfo) {
    ...
    static const TCHAR szWindowsXP64[] = _T("Windows XP x64");
        ...
        if (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT) {
            ...
            case 2:
                if (osvi.wProductType == VER_NT_WORKSTATION && sysi.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64)
                    rOsInfo.m_pszWinVersion = szWindowsXP64;
...
BOOL CSymEngine::GetNextStackTraceEntry(CStackTraceEntry &rEntry) {
    ...
    if (FSymFromAddr(m_hSymProcess, dwExceptionAddress, &dwDisplacement64, pSymbol)) {
        ...
        if (dwDisplacement64) {
            ...
            _stprintf_s(rEntry.m_szFunctionInfo, countof(rEntry.m_szFunctionInfo), _T("%s()+%s bytes"), rEntry.m_szFunctionName, rEntry.m_szFunctionOffset);
        } else {
    ...
    if (FSymGetLineFromAddr64(m_hSymProcess, dwExceptionAddress, &dwDisplacement32, &il)) {
        ...
        if (dwDisplacement32) {
            ...
            _stprintf_s(rEntry.m_szLineInfo, countof(rEntry.m_szLineInfo), _T("line %s+%s bytes"), rEntry.m_szLineNumber, rEntry.m_szLineOffset);
        } else {
