...
wchar *GetVolNumPart(const wchar *ArcName) {
    ...
    while (NumPtr > ArcName && *NumPtr != '.') {
        if (IsDigit(*NumPtr)) {
            // Validate the first numeric part only if it has a dot somewhere before it.
...
void NextVolumeName(wchar *ArcName, uint MaxLength, bool OldNumbering) {
    ...
    } else if (ChPtr[1] == 0 && wcslen(ArcName) < MaxLength - 3 || wcsicomp(ChPtr + 1, L"exe") == 0 || wcsicomp(ChPtr + 1, L"sfx") == 0)
...
#ifdef _WIN_ALL
    // In Windows we also need to convert characters not defined in current code page.
    // This double conversion changes them to '?', which is catched by code below.
...
static void GenArcName(wchar *ArcName, const wchar *GenerateMask, uint ArcNumber, bool &ArcNumPresent) {
    ...
    for (uint I = 0; Mask[I] != 0; I++) {
        ...
        if (CurChar == 'N') {
            ...
            // of archive number. We'll replace them by actual number later in this function.
            if (NCount < Digits) {
