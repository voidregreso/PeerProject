...
#ifdef _WIN_ALL
        if (!ShortNameChanged) {
            // Avoid the infinite loop if UpdateExistingShortName returns the same name.
...
bool UpdateExistingShortName(const wchar *Name) {
    ...
    // We continue only if file has a short name, which does not match its long name,
    // and this short name is equal to name of file which we need to create.
