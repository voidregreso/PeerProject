...
size_t Archive::ReadHeader() {
    // Once we failed to decrypt an encrypted block, there is no reason to attempt to do it further.
    // We'll never be successful and only generate endless errors.
...
size_t Archive::ReadHeader15() {
    ...
    case HEAD_MAIN:
        ...
        *(BaseBlock *)&MainHead = ShortBlock;
        ...
        // Only for encrypted 3.0+ archives. 2.x archives did not have this flag,
        // so for non-encrypted archives, we'll set it later based on file attributes.
...
size_t Archive::ReadHeader50() {
    ...
    case HEAD_SERVICE: {
        ...
        *(BaseBlock *)hd = ShortBlock;
        ...
        // but it was already used in RAR 1.5 and Unpack needs to distinguish them.
...
void Archive::ConvertAttributes() {
    ...
    //  if (mask == (mode_t) -1)
    //  {
    //    // umask call returns the current umask value. Argument (022)
