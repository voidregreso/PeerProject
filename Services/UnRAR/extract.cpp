...
bool CmdExtract::ExtractCurrentFile(Archive &Arc, size_t HeaderSize, bool &Repeat) {
    ...
    if (MatchFound || (SkipSolid = Arc.Solid) != 0) {
        ...
        if (!ExtrFile && Arc.Solid) {
            ...
            // if user answered "No" to overwrite prompt when unpacking a solid archive.
        ...
        if (ExtrFile) {
            ...
            if (LinkEntry) {
                ...
                } else if (Type == FSREDIR_UNIXSYMLINK || Type == FSREDIR_WINSYMLINK || Type == FSREDIR_JUNCTION) {
                    if (FileCreateMode)
...
bool CmdExtract::ExtrGetPassword(Archive &Arc, const wchar *ArcFileName) {
    ...
    else if (!PasswordAll && !Arc.FileHead.Solid) {
