...
void RarVM::Optimize(VM_PreparedProgram *Prg) {
    ...
    for (uint I = 0; I < CodeSize; I++) {
        ...
        // Replace universal opcodes with faster byte or word only processing opcodes.
        switch(Cmd->OpCode) {
        case VM_MOV:
        ...
        // the current command with faster version which does not need to modify flags.
        ...
        // Below we'll replace universal opcodes with faster byte or word only processing opcodes,
        // which also do not modify processor flags to provide better performance.
        if (FlagsRequired)
            continue;
        switch(Cmd->OpCode) {
        case VM_ADD:
