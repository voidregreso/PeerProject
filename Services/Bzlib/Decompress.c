...
Int32 BZ2_decompress ( DState *s ) {
    ...
    switch (s->state) {
            ...
            if (nextSym == BZ_RUNA || nextSym == BZ_RUNB) {
                ...
                do {
                    /* Check that N doesn't get too big, so that es doesn't go negative.
                    	The maximum value that can be RUNA/RUNB encoded is equal to
                    	the block size (post the initial RLE), viz, 900k,
                    	so bounding N at 2 million should guard against overflow
        ...
        /* Now that we know what nblock is,
        	we can do a better sanity check on s->origPtr.
        */
